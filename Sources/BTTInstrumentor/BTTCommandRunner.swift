...
final class BTTCommandRunner {
    ...
    private func runInject() {
        ...
        for file in files {
            let injectedCount = injector.inject(file: file)
            if injectedCount > 0 {
                injectedViews += injectedCount
                injectedFile += 1
            }
        }
        
        BTTLog.success("Instrumentation completed swiftUI files \(injectedFile) swiftUI views \(injectedViews) time taken : 40 ms")
    }
    ...
    private func revertSwiftFiles(
        ...
        let files   = resolver.getSwiftFiles(for: target, in: xcodeprojPath)
        BTTLog.verbose("Reverting \(files.count) file(s) for target: \(target)")
        var removedFiles = 0
        var removedViews = 0
        for file in files {
            let revertedCount = injector.revert(file: file)
            if revertedCount > 0 {
                removedFiles += 1
                removedViews += revertedCount
            }
        }
        
        BTTLog.success("Instrumentation reverted swiftUI files \(removedFiles) swiftUI views \(removedViews) time taken : 40 mili sec")
    }
